...
def create_order(cart):
    order, created = Order.objects.get_or_create(
        user=cart.user,
        order_id=str(uuid.uuid4()),  # Generate a new unique order ID
        payment_status="Pending",  # Default to Pending since no payment gateway
        shipping_address=cart.user.profile.shipping_address,
        payment_mode="None",  # No payment gateway
